...
// wraps only the aclnnTqCompressLatent host launch. SLOT_PAD=320 hardcoded (GLM-5.1).
at::Tensor tq_compress_latent(const at::Tensor &latent, const at::Tensor &centroids)
{
    int64_t N = latent.size(0);
    at::Tensor slot = at::empty({N, 320}, latent.options().dtype(at::kByte));
    EXEC_NPU_CMD(aclnnTqCompressLatent, latent, centroids, slot);
    return slot;
}
