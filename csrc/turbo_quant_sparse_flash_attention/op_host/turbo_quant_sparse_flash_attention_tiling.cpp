...
ge::graphStatus QSFATilingCheck::CheckFeatureMlaAntiquantShapeSparseAndHeadDim() const
{
    OP_CHECK_IF(sparseBlockSize_ <= 0 || (sparseBlockSize_ & (sparseBlockSize_ - 1)) != 0 || sparseBlockSize_ > 16,
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "sparseBlockSize_",
            std::to_string(sparseBlockSize_).c_str(),
            "group num should be in 1, 2, 4, 8, 16."),
        return ge::GRAPH_FAILED);

    OP_CHECK_IF(qHeadDim_ <= ropeHeadDim_,
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "qHeadDim_",
            std::to_string(qHeadDim_).c_str(), "q_head_dim should be greater than rope_head_dim."),
        return ge::GRAPH_FAILED);
...
ge::graphStatus QSFATilingCheck::CheckFeatureMlaAntiquantAttr() const
{
    ...
    OP_CHECK_IF(tileSize_ <= 0,
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "tile_size",
            std::to_string(tileSize_).c_str(), "tile_size should be positive."),
        return ge::GRAPH_FAILED);

    OP_CHECK_IF(ropeHeadDim_ <= 0,
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "rope",
            std::to_string(ropeHeadDim_).c_str(), "rope_head_dim should be positive."),
        return ge::GRAPH_FAILED);
...
ge::graphStatus QSFAInfoParser::GetOpName()
{
    if (context_->GetNodeName() == nullptr) {
        OP_LOGE_WITH_INVALID_INPUT("TurboQuantSparseFlashAttention", "opName");
        return ge::GRAPH_FAILED;
    }
...
{
    if (context_ == nullptr) {
        OP_LOGE_WITH_INVALID_INPUT("TurboQuantSparseFlashAttention", "tiling context");
        return ge::GRAPH_FAILED;
    }
