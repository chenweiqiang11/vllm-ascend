...
public:
    ...
    __aicore__ inline void InitVecBlock(TPipe *pipe, const TqcSparseFlashAttentionTilingDataMla *__restrict tiling,
...
__simd_vf__ void AntiquantVFImplFp8D448(__ubuf__ int8_t* ubSrcAddr, __ubuf__ Q_T* ubDstAddr, // output first
    __ubuf__ float* ubScaleSrcAddr, uint32_t dealRowCount, uint32_t nopeDim, uint32_t tileSize, uint32_t combineDim)
{
...
__aicore__ inline void AntiquantVFFp8D448(LocalTensor<Q_T>& outputUb,  LocalTensor<KV_T>& inputUb,
    uint32_t dealRowCount, ConstInfo &constInfo)
{
    __ubuf__ int8_t* ubSrcAddr = (__ubuf__ int8_t*)(inputUb.GetPhyAddr());
    __ubuf__ Q_T* ubDstAddr = (__ubuf__ Q_T*)(outputUb.GetPhyAddr());
    __ubuf__ float* ubScaleAddr = (__ubuf__ float*)(inputUb[
        constInfo.dSizeNope + constInfo.dSizeRope * sizeof(Q_T)].GetPhyAddr());

    AntiquantVFImplFp8D448<Q_T, KV_T>(ubSrcAddr, ubDstAddr, ubScaleAddr, dealRowCount, constInfo.dSizeNope,
        constInfo.tileSize, Align64Func(constInfo.dSizeNope + constInfo.dSizeRope * sizeof(Q_T)));
}
    ...
    LocalTensor<KV_T> srcTensor, int64_t dealRow, ConstInfo &constInfo)
{
    ...
    AntiquantVFFp8D448<Q_T, KV_T>(antiKvTensorAsB16, srcTensor, dealRow, constInfo);
    ...
    Copy(kRopeUbNz, kRopeUb,
        ...
        static_cast<uint8_t>(dealRow), // repeatTime, 每次处理多少个block
        {
            ...
            1, // src stride
            1, // dst repeat stride
            static_cast<uint16_t>((constInfo.dSizeVInput + BLOCK_BYTE - 1) / BLOCK_BYTE)
        });
}
...
__aicore__ inline void QSFAVectorService<TEMPLATE_ARGS>::InitLocalBuffer(TPipe *pipe, ConstInfo &constInfo)
{
    ...
    tPipe->InitBuffer(stage0InQue, 2,
        ((constInfo.dSizeVInput + BLOCK_BYTE - 1) / BLOCK_BYTE * BLOCK_BYTE) * 16 * sizeof(KV_T));
    tPipe->InitBuffer(stage0OutQue, 2, constInfo.dSize * (16 + 1) * sizeof(Q_T));
    ...
    CVSharedParams &sharedParams, int32_t aicIdx, uint8_t subBlockIdx)
{
    ...
    sharedParams.dSize = sparseAttnSharedkvBaseParams.headDim + sparseAttnSharedkvBaseParams.ropeHeadDim;
